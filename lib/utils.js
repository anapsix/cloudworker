...
module.exports.extractKVBindings = (kvSetFlags, kvFileFlags, kvRedisFlags) => {
  const bindings = {}

  const filepaths = extractKVPaths(kvFileFlags)

  const redispaths = extractKVPaths(kvRedisFlags)

  for (const [variable, path] of Object.entries(filepaths)) {
    bindings[variable] = new KeyValueStore(path)
  }

  for (const [variable, path] of Object.entries(redispaths)) {
    bindings[variable] = new KeyValueStore(path)
  }
