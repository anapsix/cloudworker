...
describe('cloudworker-e2e', async () => {
  ...
  test('formData', async (cb) => {
    const script = `
      addEventListener('fetch', async event => {
        const incoming = event.request
        const form = await incoming.formData()
        event.respondWith(new Response(form.get('test') + form.get('value')))
      })
    `

    const server = new Cloudworker(script).listen(8080)
    const res = await axios.post('http://localhost:8080', 'test=data&value=success', defaultAxiosOpts)
    expect(res.data).toEqual('datasuccess')
